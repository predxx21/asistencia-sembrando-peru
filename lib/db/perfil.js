  ...
  try {
    ...
    try {
      profile = await prisma.profile.findUnique({
        where: { id: userId },
        select: { id: true, nombre: true, apellido: true, rol: true, avatarUrl: true },
      });
    } catch (errSelect) {
      console.warn('Fallback select sin avatarUrl:', errSelect?.message);
      profile = await prisma.profile.findUnique({
        where: { id: userId },
...
// Actualiza los datos editables del perfil (nombre/apellido/avatarUrl) de un usuario
// ya autenticado. Se usa desde la pantalla "Editar Perfil".
export async function actualizarPerfil({ id, nombre, apellido, avatarUrl }) {
  try {
    const dataToUpdate = {};
    ...
    // nombre/apellido/avatarUrl se vea de inmediato sin esperar al TTL.
